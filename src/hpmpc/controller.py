...
class Controller:
    ...
    def read_sensors(self) -> dict[str, Any]:
        e = self.cfg.entities
        ids = [e.indoor_temp, e.outdoor_temp, e.wind_speed, e.supply_temp, e.price, e.offset_output]
    ...
    def _limit(self, target: float) -> tuple[float, list[str]]:
        c = self.cfg.control
        notes: list[str] = []
    ...
    def step(self, now: datetime | None = None, apply: bool | None = None) -> dict[str, Any]:
        ...
        if problems:
            ...
            fallback, notes = self._limit(self.cfg.control.fallback_offset)
        ...
        offset, notes = self._limit(target)
        report["notes"].extend(notes)
